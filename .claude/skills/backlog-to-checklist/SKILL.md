...
1. `get_issues` với các tham số sau (BẮT BUỘC):
   - `projectId=[<id>]` — KHÔNG dùng `keyword` để filter người phụ trách
   - `assigneeId=[<id>]` — luôn dùng ID số, tra từ `get_issue` hoặc hỏi người dùng nếu chưa biết
   - `statusId=[<id>]` — nếu lọc theo trạng thái
   - `parentIssueId=[<id>]` — nếu người dùng muốn lọc theo 親課題 (ticket cha); tra ID bằng `get_issue` với issueKey của ticket cha
   - `count=100` — luôn đặt 100 để không bỏ sót; nếu kết quả trả về đúng 100 thì gọi thêm lần 2 với `offset=100` (pagination)
   - `order="asc"` + `sort="issueType"` — để kết quả ổn định theo số ticket

2. Trong `fields` của `get_issues`, luôn lấy thêm `parentIssueId` để biết ticket thuộc 親課題 nào.
3. Sau khi có danh sách ticket, gom các `parentIssueId` duy nhất → gọi `get_issue` cho từng parent để lấy `issueKey` + `summary` (dùng làm nhãn nhóm).
4. Với mỗi ticket con, gọi `get_issue_comments` để lấy comment.

Giữ lại: issueKey, tiêu đề ticket, màn hình (screen), assignee, status, ngày, **親課題 key + tên**. Dùng dựng header mỗi bảng ở Bước 5.
...
Xuất MỘT file Markdown vào folder `output/` trong thư mục dự án, đặt tên `Checklist_<loại>_<assignee>_YYYY-MM-DD.md`. Mỗi ticket một phần:
    ...
    - 親課題: [POS_UI-XX] Tên 親課題  ← luôn hiển thị; nếu không có thì ghi "なし"
...
Nếu có nhiều 親課題 khác nhau trong một file, **sắp xếp các ticket theo 親課題** (gom nhóm cùng parent liền nhau) và thêm dòng tiêu đề phân cách:

    ---
    # 親課題: [POS_UI-XX] Tên 親課題
    ---
